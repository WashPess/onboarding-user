...
public class UserModel {
	...
	public UserModel(String email, String document, String firsName, String lastName, String nickname, String password,
			String confirmPassword) {
	...
	private Response validPassword() {
		...
		if (!RegexCompile.Password.matcher(this.password).matches()) {
			this.logger.error(
					"A senha precisa ter mais de 8 caracteres, pelo menos 1 caractere maiúsculo, 1 minúsculo e um especial.");
			return Response.error(400, "USE030",
					"A senha precisa ter mais de 8 caracteres, pelo menos 1 caractere maiúsculo, 1 minúsculo e um especial.");
		}
	...
	public String toString() {
		return "User[id=%d, uuid=%s, email=%s, document=%s, firstName='%s', lastName='%s', fullName=%s, nickname=%s, password=%s, status=%s]"
				.formatted(this.id, this.uuid, this.email, this.document, this.firstName, this.lastName, this.fullName,
						this.nickname, this.password, this.status);
	}
}
